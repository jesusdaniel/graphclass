/*********************************************************************
** Program: Inner produce using R and using C
**********************************************************************
** TO COMPILE:
...
double group_lasso_node_penalty(double *Db, int *N, double *gl){
    int i,j;
    double norm_node;
    gl[0] = 0;
    for(i=0; i <N[0]; i++) {
...
double l1l2_node_soft_thresholding(double *b, int *N, double *lambda1, double * lambda2) {
    int i,j,p;
    p = N[0]*(N[0]-1);
    for(i=0; i<p; i++){
        if(b[i]>lambda1[0]){
...
double l2_node_soft_ghtesholding(double *Db, int *N, double *lambda){
    int i,j;
    double norm_node;
    double t;
    for(i=0; i <N[0]; i++) {
