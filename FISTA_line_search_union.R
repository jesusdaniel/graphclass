...
fista_line_search_union <- function(proximal_f,b_step,f,grad_f,penalty, x_start, b_start,
                              X,Y,D,
                              ID = "NULL", verbose = F,
  ...
  best_f = f(Xbeta,b) + penalty(x_start,b);  best_prox = NULL
  newf = best_f
  ...
  while(k<=5 | ((crit_f > TOLERANCE  & criterion > TOLERANCE) & (k < MAX_STEPS) &
    ...
    ######## line search ###################
    repeat {
      z=y-tk*grad_f(X%*%crossprod(D,y),b)
