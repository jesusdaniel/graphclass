# STATUS
fista_line_search <- function(proximal_f,b_step,f,grad_f,penalty, x_start, b_start,
                              X,Y,D,
                              ID = "NULL", verbose = F,
  ...
  b = b_step(x_start,b_start)
  best_beta = x_start;  best_b = b;   
  best_f = f(x_start,b) + penalty(x_start,b);  best_prox = NULL
  newf = best_f
  ...
  while(k<=5 | ((crit_f > TOLERANCE  & criterion > TOLERANCE) & (k < MAX_STEPS) &
    ...
    repeat {
      ...
      z = prox$x
      if(f(z,b) <= f(y,b) +t(grad_f(y,b))%*%(z-y) + 1/(2*tk) * norm_s(z-y)^2)
    ...
    fs = c(fs, newf)
    if(verbose)  cat(sprintf("---%s - Iteration %d. Sparse = %d.  Crit_f = %.4f. tk = %.3f. F = %.4f. t=%2f.\n",
                ID,k,sum(xk1==0),crit_f,tk,newf,proc.time()[1]-time_start))
